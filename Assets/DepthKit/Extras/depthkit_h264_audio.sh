...
if [ "$#" -ne 6 ]; then 
    ...
    echo "$ ./depthkit_h264_audio.sh <source_movie_path> <image_sequence_path> <start_frame> <end_frame> <framerate> <output_movie_name>"
    exit 1
fi
...
ffmpeg -y -start_number $START_FRAME -f image2 -r ${FRAME_RATE} -i "${SEQUENCE_PATH}/save.%05d.png" -i "${INPUT_MOVIE_PATH}.WAV" -codec:v libx264 -crf 10 -b:v 1M -c:a libvorbis -r $FRAME_RATE "${OUTPUT_MOVIE_PATH}.mov"
